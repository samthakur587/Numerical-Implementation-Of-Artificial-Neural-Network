...
class neural_network():
    ...
    def load_data(self,files):
        ...
        examples = data_x
        labels = data_y
        return classes,examples,labels
...
def main():
    ...
    model = neural_network(layer_dims=[12288,20,7,5,1],keep_prob=0.75,regularization=None,lambd=1,learning_rate=0.01,decay_rate=0.00001,decay='descrite',print_cost=True,optimizer='gd',activation_function='relu',epoch=501,seed=1)
    model.fit(x_train,y_train)
    classes ,test_exp , labels = model.load_data('data/test_catvnoncat.h5')
    y_pred = model.evaluate(x_test)
    #test_img = np.expand_dims(x_test.T[0],axis=1)
    prediction = model.evaluate(x_test[:,:9])
    plt.figure(figsize=(20,20))
    for i in range(1,10):

        plt.subplot(3,3,i)
        plt.imshow(test_exp[i-1])
        plt.title('prediction is : '+str(classes[prediction[0][i-1]]))
        plt.axis('off')
    plt.show()
